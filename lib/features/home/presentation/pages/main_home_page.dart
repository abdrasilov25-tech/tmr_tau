...
class _MainHomePageState extends State<MainHomePage> {
  ...
  Future<void> _createPostWithPlaceholderMedia() async {
    final userId = _currentUserId;
    if (userId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Войдите, чтобы публиковать')),
      );
      return;
    }

    const placeholderPhoto = 'https://picsum.photos/seed/tmr_tau_photo/900/900';
    const placeholderVideo =
        'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4';

    final result = await showModalBottomSheet<_CreatePostResult>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.black,
      constraints: const BoxConstraints(maxWidth: 560),
      builder: (_) => _CreatePostSheet(
        placeholderPhotoUrl: placeholderPhoto,
    ...
    try {
      final created = await context.read<PostRepository>().createPost(
            userId: userId,
            caption: result.caption,
            kind: 'publication',
            imageUrl: result.media == _MediaType.photo ? placeholderPhoto : '',
            videoUrl: result.media == _MediaType.video ? placeholderVideo : null,
